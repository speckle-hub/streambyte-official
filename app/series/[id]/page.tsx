        ...
        <div className="lg:col-span-3 flex flex-col gap-8">
           ...
           <div className="grid grid-cols-1 gap-4">
              {episodes.map((ep) => (
                <Link
                  key={ep.id}
                  href={`/player?id=${ep.id}&type=series`}
                  className="flex items-center gap-6 p-4 rounded-2xl bg-zinc-900/50 border border-zinc-800 hover:bg-zinc-900 hover:border-primary/50 transition-all group"
                >
                  <div className="relative aspect-video w-40 flex-shrink-0 overflow-hidden rounded-lg bg-zinc-800">
                    {ep.thumbnail ? (
                      ...
                      <div className="flex h-full w-full items-center justify-center text-zinc-600 font-bold">NO THUMB</div>
                    )}
                  ...
                  <div className="flex flex-col gap-1 flex-1">
                    ...
                    {ep.overview && <p className="text-sm text-zinc-500 line-clamp-2 mt-1 leading-relaxed font-medium">{ep.overview}</p>}
                    <div className="flex items-center gap-4 mt-2 text-xs text-zinc-600 font-bold">
                       <span>{ep.released ? new Date(ep.released).toLocaleDateString() : 'Unknown Date'}</span>
                    </div>
                  </div>
                </Link>
              ))}
